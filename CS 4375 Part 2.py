...
class NeuralNet:
    def __init__(self, dataFile, header=True, h=4):
        ...
        raw_input = pd.read_csv("audit_risk.csv")
        ...
        tcols = len(self.test_dataset.columns)
        trows = len(self.test_dataset.index)
        #
    ...
    def ReLu(self,x):
        return np.array([self.ReLu_derivative(xi) for xi in x])
    ...
    def forward_pass(self, activation="tanh"):
        # pass our inputs through our neural network
        in_hidden = np.dot(self.X, self.W_hidden) + self.Wb_hidden
        # TODO: I have coded the sigmoid activation, you have to do the rest
        if activation == "tanh":
    ...
    def backward_pass(self, out, activation = "tanh"):
        # pass our inputs through our neural network
        self.compute_output_delta(out, activation)
        self.compute_hidden_delta(activation)
        
    def backward_pass(self, out, activation = "ReLu"):
        # pass our inputs through our neural network
        self.compute_output_delta(out, activation)
        self.compute_hidden_delta(activation)    
